(function($) {
	...
	// Adjust slider position dynamically on window resize
	function adjustSliderPosition() {
		const headerHeight = $('.main-header').outerHeight();
		$('#main-slider').css('padding-top', headerHeight + 'px');
	}

	$(window).on('resize', adjustSliderPosition);
	$(document).ready(adjustSliderPosition);

	// Ensure the slider functionality is properly initialized
	function initializeSlider() {
		...
		if (!slider || items.length === 0 || !next || !prev) {
	...
	// Call the slider initialization function on document load
